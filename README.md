# DSO: Direct Sparse Odometry (standalone + ROS support)

## We unified [dso](https://github.com/JakobEngel/dso) and [dso_ros](https://github.com/JakobEngel/dso_ros) so that anyone (who may not ne familiar with linux build system) can use DSO for tutorial.

### 1. Prerequisite
#### Docker (highly recommended)
**User's are highly recommended to use Docker container to use this repository.**
1. Original DSO is quite old; It will yield compatibility issues with your current installed library.
2. There are **pre-built docker image for this repository**: please check out my **[Docker project repository](link)**.

#### Download datasets
Download a dataset from [https://vision.in.tum.de/mono-dataset](https://vision.in.tum.de/mono-dataset).


### 2. Installation
**Please use docker!** I recommend not to install everything on your own.
If you want to build and install the package locally, please note that installation process is the same as the original.


### 3 Usage

#### DSO (non-ROS version)
After navigating yourself to `./dso/build`, run:
```
bin/dso_dataset \
    ...
    mode=0
```
replace `XXXXX/sequence_XX` with the actual path of dataset, downloaded from [https://vision.in.tum.de/mono-dataset](https://vision.in.tum.de/mono-dataset).

#### DSO (ROS1 version)
First source setup.bash of your workspace's devel folder (e.g. `source ~/catkin_ws/devel/setup.bash`), run:
```
